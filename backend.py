...
class PythonCode:
    ...
    def _translate_single_ir_instruction(self, instruccion, indent_level): #indent pq pues python super imp indent no?
        parts = instruccion.split(" ", 1)
        opcode = parts[0]
        args = parts[1] if len(parts) > 1 else ""

        def _translate_operand(operand_str, used_in_condition=False):
        ...
        if opcode == "ASSIGN":
            ...
            translated_line = f"{dest} = {_translate_operand(op1)} + {_translate_operand(op2)}"
        elif opcode == "SUB":
            dest, ops = [s.strip() for s in args.split("=", 1)]
            op1, op2 = [s.strip() for s in ops.split(",")]
            translated_line = f"{dest} = {_translate_operand(op1)} - {_translate_operand(op2)}"
            ...
            dest, ops = [s.strip() for s in args.split("=", 1)]
            op1, op2 = [s.strip() for s in ops.split(",")]
            translated_line = f"{dest} = {_translate_operand(op1, used_in_condition=True)} and {_translate_operand(op2, used_in_condition=True)}"
        elif opcode == "OR":
            dest, ops = [s.strip() for s in args.split("=", 1)]
            op1, op2 = [s.strip() for s in ops.split(",")]
            translated_line = f"{dest} = {_translate_operand(op1, used_in_condition=True)} or {_translate_operand(op2, used_in_condition=True)}"
        elif opcode == "NOT":
            dest, op = [s.strip() for s in args.split("=", 1)]
            translated_line = f"{dest} = not {_translate_operand(op, used_in_condition=True)}"
        elif opcode == "PRINT":
